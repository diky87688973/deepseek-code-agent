...
def _build_direct_preview_message(script_name: str, result: dict, user_text: str) -> Optional[str]:
    """已停用：写类预览不再直出主对话（恒返回 None，保留调用点兼容）。"""
    _ = (script_name, result, user_text)
    return None


def preflight_write_tool(
    conversation_id: str,
    script_name: str,
    args: Dict[str, Any],
    *,
    ...
    todo_list_mod: Any = None,
    audit_block_msg: str = "",
) -> Optional[Dict[str, Any]]:
    """写工具执行前完整 HostPolicy 门控。返回 None 表示放行；否则返回拒绝 result。"""
    sn = str(script_name or "")
    if sn not in WRITE_TOOL_SCRIPTS:
        return None
    cid = str(conversation_id or "").strip()
    from agent_v4.runtime.host_policy import HostPolicy, _CONVERSATION_PREVIEWED

    previewed = (
        previewed_files
        if previewed_files is not None
        else (_CONVERSATION_PREVIEWED.setdefault(cid, {}) if cid else {})
    )
    written = written_files if written_files is not None else {}
    if todo_list_mod is None:
        import todo_list as todo_list_mod  # type: ignore
    msg = str(audit_block_msg or "").strip() or _AUDIT_WRITE_BLOCK_MSG
    return HostPolicy().gate_write_tool(
        cid,
        sn,
        args,
        ...
        current_mode=CONVERSATION_MODES.get(cid, ""),
        audit_only=bool(CONVERSATION_AUDIT_ONLY.get(cid)),
        audit_block_msg=msg,
        todo_list_mod=todo_list_mod,
    )
...
def _execute_tool_script_stoppable(
    ...
    *,
    ...
    skip_write_preflight: bool = False,
    step_title: str = "",
    previewed_files: Optional[Dict[str, str]] = None,
    written_files: Optional[Dict[str, str]] = None,
    todo_list_mod: Any = None,
    ...
    def _run() -> None:
        try:
            holder["r"] = execute_tool_script(
                script_name,
                args,
                conversation_id=conversation_id,
                skip_write_preflight=skip_write_preflight,
...
def attach_tool_help_on_failure(script_name: str, mod: Optional[Any], result: dict) -> dict:
    """工具返回 ok=false 时附带 tool_help；简单错误用精简版，复杂错误只靠 catalog。"""
    if not isinstance(result, dict) or result.get("ok"):
        return result
...
def execute_tool_script(
    ...
    *,
    ...
    cid = str(conversation_id or "").strip()
    # 宿主专用工具：真实现（早于 mode 注入，避免 run_type 查询被污染）
    hooked = _host_tool_hook_result(script_name, args, cid)
    if hooked is not None:
        return hooked
    # 完整写门控（Plan/Execute/todo/预览/质量）；进度线程可 skip 以避免重复
    if not skip_write_preflight:
        block = preflight_write_tool(
            cid,
            script_name,
            args,
            step_title=step_title,
        ...
        if block is not None:
            return attach_tool_help_on_failure(script_name, None, block)
