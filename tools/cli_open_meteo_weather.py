#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Open-Meteo 免费天气查询（HTTPS，无需 API Key）。"""
...
def _emit(ok: bool, data: dict | None = None, error: dict | None = None) -> None:
    print(json.dumps({"ok": ok, "data": data, "error": error}, ensure_ascii=False))


def _http_json(url: str, timeout_sec: int) -> dict[str, Any]:
    req = urllib.request.Request(url, headers={"User-Agent": "cli-open-meteo-weather/1"})
...
def build_parser() -> argparse.ArgumentParser:
    p = _HelpFulParser(description="Open-Meteo 天气查询（免费）")
    p.add_argument("--location", default="", help="城市或地区名；与经纬度二选一")
    p.add_argument("--latitude", default="", help="与 --longitude 成对")
    p.add_argument("--longitude", default="", help="与 --latitude 成对")
    p.add_argument("--ip", default="", help="可选。用于 IP 定位；localhost/127.0.0.1/0.0.0.0 将视为未传")
    p.add_argument("--forecastDays", type=int, default=BUILTIN_FORECAST_DAYS, help="预报天数 1-7")
    p.add_argument("--timeoutSec", type=int, default=BUILTIN_TIMEOUT_SEC, help="HTTP 超时秒数")
    p.add_argument("--jsonOut", action="store_true", help="输出统一 JSON")
    return p


def _open_meteo_envelope(args: argparse.Namespace, parser: argparse.ArgumentParser) -> dict:
    timeout_sec = max(5, int(args.timeoutSec or BUILTIN_TIMEOUT_SEC))
    loc_input = (args.location or "").strip()
    days = int(args.forecastDays) if args.forecastDays else BUILTIN_FORECAST_DAYS
    days = max(1, min(7, days))

    lat_s = (args.latitude or "").strip()
    lon_s = (args.longitude or "").strip()
    ip_for_lookup = _normalize_ip_for_lookup(args.ip or "")
    ...
    if lat_s and lon_s:
        ...
        if alias is not None:
            ...
            geo = _http_json(f"{GEO_URL}?{q}", timeout_sec)
        ...
        ip_geo = _lookup_ip_region(ip_for_lookup, timeout_sec)
        if ip_geo is None:
            return {
                "ok": False,
                "data": None,
    ...
    fc = _http_json(f"{FC_URL}?{q2}", timeout_sec)
...
def agent_main(
    *,
    ...
    args = argparse.Namespace(
        ...
        longitude=longitude,
        ip=ip,
        forecastDays=forecast_days if forecast_days is not None else BUILTIN_FORECAST_DAYS,
        timeoutSec=timeout_sec if timeout_sec is not None else BUILTIN_TIMEOUT_SEC,
        jsonOut=True,
    )
    try:
        return _open_meteo_envelope(args, parser)
        ...
        return {"ok": False, "data": None, "error": {"type": e.__class__.__name__, "message": msg, "hint": "", "retryable": False}}
...
def main() -> None:
    ...
    try:
        out = _open_meteo_envelope(args, parser)
        _emit(out["ok"], out["data"], out["error"])
    except urllib.error.HTTPError as e:
        _emit(False, None, {"type": "HTTPError", "message": str(e), "hint": "稍后重试", "retryable": True})
    except urllib.error.URLError as e:
        _emit(False, None, {"type": "URLError", "message": str(e.reason), "hint": "检查网络或 DNS", "retryable": True})
    except Exception as e:
        e.args = (str(e) + "\n\n--help:\n" + _capture_help(parser),)
        _emit(False, None, {"type": e.__class__.__name__, "message": str(e), "hint": "", "retryable": False})
