# -*- coding: utf-8 -*-
"""可灵 Kling AI 视频生成工具 — 官方 HTTP API。

支持：文生视频(text2video) / 图生视频(image2video) / 查询任务结果
API 文档：https://klingai.com/document-api/apiReference/model/textToVideo
"""
...
def _kling_request(method: str, path: str, body: Optional[dict] = None, timeout: int = 120) -> dict:
    ...
    try:
        ...
        msg = bt
        try:
            msg = json.loads(bt).get("message", bt)
...
def _action_text2video(prompt: str, model_name: str = "kling-v1", duration: str = "5",
    ...
    if r.get("ok") and isinstance(r.get("data"), dict):
        ...
        # 归档 prompt 到任务目录
        _save_text2video_meta(d.get("task_id", ""), prompt, body)
    return r


def _action_query_result(task_id: str = "", external_task_id: str = "", auto_download: bool = True) -> dict:
    """查询任务结果（非阻塞，一次查询返回当前状态）。"""
    if task_id:
        path = f"/v1/videos/text2video/{task_id}"
    elif external_task_id:
        path = f"/v1/videos/text2video?external_task_id={external_task_id}"
    else:
        return ac.err(ValueError("请提供 task_id 或 external_task_id"))
    r = _kling_request("GET", path, timeout=30)
    if r.get("ok") and isinstance(r.get("data"), dict):
        d = r["data"].get("data", {})
        ...
        if videos:
            ...
            if urls:
                md = [f"✅ **生成成功！**\n（请直接输出本消息内容，不要改写格式，否则视频无法播放）\n"]
                for u in urls:
                    md.append(f"![播放视频]({u})")
                    if auto_download:
                        lp = _download_video(u, ret_task_id)
                        if lp:
                            md.append(f"📁 已保存: `{lp}`")
                r["data"]["message"] = "\n\n".join(md)
    return r
...
def _download_video(url: str, task_id: str) -> str:
    """下载视频到 WORKSPACE_DIR/kling_tasks/{task_id}/。"""
    try:
...
def _save_text2video_meta(task_id: str, prompt: str, body: dict):
    """提交任务后将 prompt 归档到任务目录。"""
    if not task_id:
        return
    try:
        from pathlib import Path
        ...
        md_path = task_dir / f"kling_{task_id}_prompt.md"
...
def agent_main(*, action: str = "text2video", prompt: str = "",
    ...
    try:
        ...
        # Plan 模式下禁止执行生成类任务（消耗积分）
        _generate_actions = {"text2video", "text2image", "image2video", "image2image",
                             "image_upscale", "frames2video", "multiframe2video",
                             "multimodal2video"}
        ...
        if action == "text2video":
            return _action_text2video(prompt, model_name, duration, mode, sound, aspect_ratio, negative_prompt, cfg_scale, multi_shot, shot_type, multi_prompt, external_task_id, callback_url, camera_control, watermark_enabled)
        elif action == "query_result":
            return _action_query_result(task_id, external_task_id)
...
def main() -> None:
    p = argparse.ArgumentParser(description="可灵 Kling AI 视频生成")
    p.add_argument("--action", default="text2video", help="操作类型：text2video / query_result")
    p.add_argument("--prompt", default="", help="提示词")
    p.add_argument("--model_name", default="kling-v1", help="模型名称")
    p.add_argument("--duration", default="5", help="视频时长(秒)")
    p.add_argument("--mode", default="std", help="模式：std/pro/4k")
    p.add_argument("--sound", default="off", help="是否生成声音：on/off")
    ...
    p.add_argument("--shot_type", default="", help="分镜方式：customize/intelligence")
