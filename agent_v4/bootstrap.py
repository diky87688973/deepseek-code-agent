...
if getattr(sys, "frozen", False):
    ...
    _base = Path(__file__).resolve().parent.parent  # agent_v4 -> project root
AGENT_ROOT = _base
...
# 安全删除回收站：与 main_tray / file_ops 共用 AGENT_RECYCLE_ROOT（单一目录）
_recycle_env = str(os.environ.get("AGENT_RECYCLE_ROOT") or "").strip()
if _recycle_env:
    RECYCLE_ROOT = Path(_recycle_env).expanduser().resolve()
else:
    RECYCLE_ROOT = (DATA_ROOT / "AI_安全删除回收站").resolve()
    os.environ["AGENT_RECYCLE_ROOT"] = str(RECYCLE_ROOT)
try:
...
_agent_common_mod.configure_file_backup_root(DATA_ROOT / "temp" / "file_backup")
# ── Agent 会话加载 ──
try:
...
# 视觉：单图 token 估算上限（DeepSeek 官方每图最多 384）与嵌图预算/缩放边长
IMAGE_TOKENS_PER_IMAGE = int(_CONTEXT_CFG.get("AGENT_IMAGE_TOKENS_PER_IMAGE") or 384)
VISION_EMBED_MAX_BYTES = int(_CONTEXT_CFG.get("AGENT_VISION_EMBED_MAX_BYTES") or (24 * 1024 * 1024))
VISION_EMBED_MAX_SIDE = int(_CONTEXT_CFG.get("AGENT_VISION_EMBED_MAX_SIDE") or 800)
