    ...
    *,
    ...
    try:
        ...
        if confirm_id:
            ...
            if stored.get("action") != "write_file":
                return {"ok": False, "data": None, "error": {"type": "CrossToolConfirmId", "message": f"该 confirm_id 由 {stored.get('action','?')} 创建，write_file 无法消费，请使用创建该 ID 的原始工具提交"}}
