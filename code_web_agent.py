#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Code Web Agent: OpenAI-compatible Chat Completions + tool library (cli_*.py entrypoints).
...
- 工具调用失败时（ok 非 true）可向 DATA_ROOT 下 debug 目录写入 JSON 记录；CODE_WEB_AGENT_TOOL_DEBUG=0 关闭。
"""
...
_dr = str(AGENT_CONFIG.get("CODE_WEB_AGENT_DATA_DIR") or "").strip()
if _dr:
...
# ── 知识库配置 ──
_KB_DIR_STR = str(AGENT_CONFIG.get("KNOWLEDGE_BASE_DIR") or "").strip()
...
UI_STATE_FILE = DATA_ROOT / "cache" / "ui_state.json"
_skf = str(AGENT_CONFIG.get("CODE_WEB_AGENT_SESSION_KEY_FILE") or "").strip()
...
# ---------- 控制台调试：stderr 输出 JSON 行（SSE 事件、工具完整入参/出参）。CODE_WEB_AGENT_CONSOLE_LOG=0 关闭 ----------
def _agent_console_log_enabled() -> bool:
    v = os.environ.get("CODE_WEB_AGENT_CONSOLE_LOG", "1").strip().lower()
...
def _tool_debug_file_enabled() -> bool:
    v = os.environ.get("CODE_WEB_AGENT_TOOL_DEBUG", "1").strip().lower()
    return v not in ("0", "false", "no", "off")
...
def _encrypt_session_payload(plain: bytes) -> Dict[str, Any]:
    mode = str(AGENT_CONFIG.get("CODE_WEB_AGENT_SESSION_ENCRYPTION") or "auto").strip().lower() or "auto"
...
def _save_conversation(cid: str, messages: List[Dict[str, Any]]) -> None:
    if not cid:
        return
    try:
        ...
        envelope = _encrypt_session_payload(plain)
        if envelope is None:
            # none模式：直接存明文JSON
            fp.write_text(plain.decode("utf-8"), encoding="utf-8")
        else:
...
def _save_ui_state(state: Dict[str, Any]) -> None:
    try:
        ...
        envelope = _encrypt_session_payload(plain)
        UI_STATE_FILE.write_text(json.dumps(envelope, ensure_ascii=False), encoding="utf-8")
    except Exception as e:
        print(f"WARN: failed to save ui state: {e}", file=sys.stderr, flush=True)
...
def chat_sessions():
    ...
    for fp in files[:100]:
        ...
        title = title_by_id.get(cid) or _fallback_title_from_messages(cid, list(messages))
        try:
    ...
    # 按日期分组展示依赖「同 date_group 在列表中连续」；仅按 mtime 全局排序会产生同一日期被穿插、菜单重复打组标题
