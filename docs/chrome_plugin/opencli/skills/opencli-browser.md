...
### Key distinction: lease vs tab vs window

| action | what it does | tab/window affected? |
|--------|-------------|---------------------|
| `browser <session> close` | Release the CDP lease — session ends | No — tab and window stay open |
| `browser <session> unbind` | Detach a bound session | No — tab and window stay open |
| `browser <session> tab close <targetId>` | Close a browser tab via CDP | Yes — closes the tab; if last tab, the browser closes the window |
| `daemon stop` | Kill the daemon process | Extension disconnects; windows/tabs remain open |

### Normal cleanup: close tabs, not windows

When you're done with a browser session:

1. List the session's tabs:
   ```bash
   opencli browser <session> tab list
   ```
2. Close each tab by page ID:
   ```bash
   opencli browser <session> tab close <page-id>
   ```
3. If the tab was the last one in its window, Edge/Chrome closes the window automatically. If the window had multiple OpenCLI tabs, repeat for each.

This is the preferred approach — it mirrors what a user would do manually.

### Fallback: close a stuck window via Windows API

Only do this when `tab close` is unavailable (e.g. the daemon stopped and the extension disconnected, leaving a stale `about:blank` window):

```python
import ctypes
user32 = ctypes.windll.user32
# Find the about:blank window
hwnd = 12345678  # from EnumWindows matching title "about:blank"
user32.SendMessageW(hwnd, 0x0010, 0, 0)  # WM_CLOSE
```

Or via PowerShell:
```powershell
(Get-Process msedge | Where-Object { $_.MainWindowTitle -match "about:blank" }).CloseMainWindow()
```

### Prevention

- Prefer `bind` over `open` when the user already has the target page open — bound sessions don't create owned windows.
- When you must `open` (which creates a new window), plan cleanup at the end: `tab list` → `tab close` for each tab.
- After `tab close`, verify with `tab list` that no tabs remain.
## See also
