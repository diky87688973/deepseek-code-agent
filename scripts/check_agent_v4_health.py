#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""agent_v4 运行栈健康检查（提示词、import、符号绑定、session_* 契约）。发版前建议执行。
...
def _should_scan(path: Path) -> bool:
    ...
    if rel.parts[0] == "tests":
        return False
    if rel.parts[0] not in ("agent_v4", "tools", "util"):
        if rel.name not in ("main_tray.py", "deepseek_code_agent.py"):
            return False
    return True


def check_no_broken_prompt_names() -> None:
    bad = re.compile(r"TOOL_AGENT_V1|TOOL_AGENT_V2_|TOOL_AGENT_V3")
    for path in ROOT.rglob("*.py"):
        if not _should_scan(path):
            continue
        text = path.read_text(encoding="utf-8", errors="replace")
        if bad.search(text):
            errors.append(f"{path.relative_to(ROOT)}: 含废弃提示词常量名（TOOL_AGENT_V1/V2/V3 等）")
...
def check_session_create_prompt_import() -> None:
    ...
    if "TOOL_AGENT_V3" in src or "TOOL_agent_v3" in src:
        errors.append("session_create: 含错误/废弃常量 TOOL_AGENT_V3")
...
def check_core_symbols() -> None:
    ...
    import agent_v4.core.context_pipeline as cp
...
def check_core_load_global_all() -> None:
    from agent_v4 import agent_core as core
    from agent_v4.core import deps
    ...
    import agent_v4.core as core_pkg

    for mi in pkgutil.iter_modules(core_pkg.__path__):
        if mi.name.startswith("_"):
            continue
        mod = importlib.import_module(f"agent_v4.core.{mi.name}")
...
def check_core_import_order() -> None:
    ...
    if re.search(r"^from agent_v4\.core\.agent_turn import", tr, re.M):
        errors.append("turn_runner.py 顶层仍 from agent_v4.core.agent_turn import（易循环导入）")
    to_clear = [k for k in list(sys.modules) if k.startswith("agent_v4")]
    for k in to_clear:
        del sys.modules[k]
    try:
        importlib.import_module("agent_v4.core.agent_turn")
    except Exception as exc:
        errors.append(f"直接 import agent_v4.core.agent_turn 失败: {exc}")
...
def check_session_tools_contract() -> None:
    ...
    import agent_v4.core.agent_turn as agent_turn_mod
...
def check_tools_agent_core_refs() -> None:
    ...
    for p in tools_dir.glob("*.py"):
        text = p.read_text(encoding="utf-8", errors="replace")
        if "agent_v4.agent_core" not in text:
            continue
        for m in re.finditer(
            r"(?:from agent_v4\.agent_core import|agent_v4\.agent_core\.)([a-zA-Z_][a-zA-Z0-9_]*)",
            text,
        ):
            sym = m.group(1)
