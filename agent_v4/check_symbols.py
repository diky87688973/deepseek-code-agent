#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""静态/运行时符号检查：agent_v4 门面 agent_core 与 routes 的引用是否齐全。"""
...
def _routes_core_refs() -> List[str]:
    ...
    # 只匹配门面别名 core.xxx，避免误匹配 agent_v4.core.attachments
    names = sorted(set(re.findall(r"(?<![.\w])core\.([a-zA-Z_][a-zA-Z0-9_]*)", src)))
...
def main() -> int:
    ...
    for mod, fn in (
        ("agent_v4.core.agent_turn", "run_agent_turn"),
        ("agent_v4.core.context_pipeline", "_maybe_schedule_summarization"),
        ("agent_v4.core.tool_runtime", "execute_tool_script"),
    ):
        for fname, name, ln in _missing_globals_in_module(mod, fn):
            errors.append(f"{mod}.{fname} L{ln}: 未定义全局 {name}")

    try:
        import deepseek_code_agent as entry  # noqa: F401

        if not hasattr(entry, "app"):
            errors.append("deepseek_code_agent 缺少 app")
    except Exception as exc:
        errors.append(f"无法 import deepseek_code_agent: {exc}")
