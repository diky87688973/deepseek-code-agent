...
#    或直接使用python：  python code_web_agent.py
```
...
├── start.bat                   # Windows 一键启动，会锁定程序目录避免篡改破坏
├── start.sh                    # Linux/macOS 一键启动，会锁定程序目录避免篡改破坏
├── python code_web_agent.py    # 直接以系统python命令运行, 则不会开启目录防护
├──
