...
def build_parser() -> argparse.ArgumentParser:
    p = _HelpFulParser(description="IP 地理位置查询（ipwho.is 免费 API）")
    p.add_argument("--ip", default="", help="可选。指定 IPv4/IPv6；省略时查询当前公网出口 IP")
    p.add_argument("--timeoutSec", type=int, default=BUILTIN_TIMEOUT_SEC, help="HTTP 超时秒数，默认 12")
...
def _http_json(url: str, timeout_sec: int) -> dict[str, Any]:
    req = urllib.request.Request(url, headers={"User-Agent": "cli-ip-geolocate/1"})
...
def agent_main(*, ip: str = "", timeout_sec: int | None = None) -> dict:
    """进程内入口；返回与 _emit 相同结构的字典，不打印。"""
...
def main() -> None:
    p = build_parser()
    args = p.parse_args()
    res = agent_main(ip=args.ip, timeout_sec=int(args.timeoutSec or BUILTIN_TIMEOUT_SEC))
    if res["ok"]:
        _emit(True, res["data"], None)
        return
    err = res.get("error") or {}
    msg = str(err.get("message", ""))
    full_msg = msg + "\n\n--help:\n" + _capture_help(p)
    _emit(False, None, {**err, "message": full_msg})
